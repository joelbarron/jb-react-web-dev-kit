...
2. Choose release mode:
   ...
   - RC: run `npm run release:rc` (merges `develop -> next` and pushes).
3. If no pending changeset exists, the script runs `npx changeset add` and commits it automatically.
4. Stable script removes `.changeset/pre.json` from `develop`/`main` if present.
5. Wait for GitHub Actions to update/create the release PR:
...
# - removes `.changeset/pre.json` from develop/main if present
# - pushes develop
...
# - if no pending changeset exists, runs `npx changeset add` and commits it
# - removes `.changeset/pre.json` from develop if present
# - merges develop into next
# - ensures `.changeset/pre.json` exists on next (rc mode)
# - pushes next
npm run release:rc
```
...
- `main` should not keep `.changeset/pre.json`, otherwise Changesets treats main as prerelease mode.
